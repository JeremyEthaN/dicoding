...
def data_preprocessing(data):
    ...
    df["Application_mode"] = scaler_Application_mode.transform(np.asarray(data["Application_mode"]).reshape(-1, 1))
    df["Application_order"] = scaler_Application_order.transform(np.asarray(data["Application_order"]).reshape(-1, 1))
    df["Course"] = scaler_Course.transform(np.asarray(data["Course"]).reshape(-1, 1))
    ...
    df["Tuition_fees_up_to_date"] = scaler_Tuition_fees_up_to_date.transform(np.asarray(data["Tuition_fees_up_to_date"]).reshape(-1, 1))
    df["Gender"] = scaler_Gender.transform(np.asarray(data["Gender"]).reshape(-1, 1))
    df["Scholarship_holder"] = scaler_Scholarship_holder.transform(np.asarray(data["Scholarship_holder"]).reshape(-1, 1))
